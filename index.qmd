...
```bash
pip install peclet          # CPU (OpenMP) wheels — every page here runs from this
```

For the GPU sections, install the CUDA family **into its own environment** — `peclet-cu13` provides
the same `peclet.*` imports as `peclet`, so the two are mutually exclusive:

```bash
python -m venv .venv-gpu && source .venv-gpu/bin/activate
pip install peclet-cu13     # CUDA 13: flow + pnm + dem + voro, and morton
```

Linux x86-64 (on Windows, inside WSL2). Only an NVIDIA driver is needed — no CUDA toolkit; the
runtime arrives as a dependency wheel. Multi-GPU/MPI and AMD/HIP are source or container builds:
see [Install & run](https://computational-chemical-engineering.github.io/peclet/DEPLOYMENT/).
...
## Start here: the 1.1.0 film

:::::: {.section-lead}
Eight minutes on what the suite does and — at length — the evidence for it: the exact solutions
first, then published experiments, then the release's own scaling record — 32 H100 GPUs carrying
1.81 billion cells at 64 % weak efficiency, with every rung of the ladder returning the same mean
velocity to one part in ten billion.

::::::

```{=html}
<!-- MUST stay inside a ```{=html} fence. Without it Pandoc treats the iframe line as a
     paragraph and emits <p><iframe></p>; Bootstrap's .ratio sizes DIRECT children only
     (`.ratio > *`), so the <p> gets the 16x9 box and the iframe falls back to its intrinsic
     300x150 — the film renders as a thumbnail. Fixed 2026-09-17. -->
<div class="ratio ratio-16x9" style="max-width:100%;margin:0 auto;">
  <iframe src="https://www.youtube.com/embed/7D5Em5zvZRI?rel=0&amp;modestbranding=1"
          title="Peclet 1.1.0 — resolve what you can, model what you must"
...
Every movie on this site is also on the channel, next to the page it came from:
[**@PecletHPC**](https://www.youtube.com/@PecletHPC) · and the documentation is at
[computational-chemical-engineering.github.io/peclet](https://computational-chemical-engineering.github.io/peclet/).
...
::::: {.g-col-12 .g-col-sm-6 .g-col-lg-4}
:::: {.gallery-card}
[![](assets/gallery/voronoi-taylor-green.jpg)](examples/voronoi-taylor-green/index.qmd)
...
::::: {.g-col-12 .g-col-sm-6 .g-col-lg-4}
:::: {.gallery-card}
[![](assets/gallery/voronoi-sphere-drag.jpg)](examples/voronoi-sphere-drag/index.qmd)
...
::::: {.g-col-12 .g-col-sm-6 .g-col-lg-4}
:::: {.gallery-card}
[![](assets/gallery/pore-mesh-redistribution.jpg)](examples/pore-mesh-redistribution/index.qmd)
...
::::: {.g-col-12 .g-col-sm-6 .g-col-lg-4}
:::: {.gallery-card}
[![](assets/gallery/pore-scale-imbibition.jpg)](examples/pore-scale-imbibition/index.qmd)
...
The GPU sections (million-particle beds, cube drums, DNS) need `peclet-cu13` (in its own venv, as
above) or a local build — each page's *Reproduce this* section gives the exact commands.
