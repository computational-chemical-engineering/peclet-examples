# peclet 1.0.0 scaling benchmark — runbook

The measurement behind the Zenodo deposit *Parallel performance of peclet.flow 1.0.0*
...
method**, driven by a body force in a triply periodic box — `peclet.flow` 1.0.0, one MPI rank per
GPU or per core.
...
The **shipped 1.0.0 defaults** — MG-PCG at rtol 1e-8, coarse-level telescoping, the `auto`
...
python -m venv v && ./v/bin/pip install "peclet-dem==1.0.0" numpy
OMP_NUM_THREADS=8 OMP_PROC_BIND=false ./v/bin/python make_bed.py     # -> bed_phi0.45_r18_s0.npz

# 2. site build of v1.0.0 on Snellius, one tree per backend (~5 min each)
