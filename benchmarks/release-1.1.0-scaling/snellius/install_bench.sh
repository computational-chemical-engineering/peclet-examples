#!/bin/bash
# ==========================================================================================
# Provisioning for the peclet 1.1.0 scaling deposit: build the RELEASED tag from source into
...
#
#   sbatch --nodes=1 --gpus-per-node=1 --ntasks-per-node=1  snellius/install_bench.sh <sha> h100 v1.1.0
#   sbatch -p genoa --gpus-per-node=0 -c 32 -t 02:00:00      snellius/install_bench.sh <sha> cpu v1.1.0
#
...
set -euo pipefail
# REF is anything git can check out -- a tag, a branch, or a COMMIT SHA. 1.1.0 was measured before
# the tag existed, so the campaign pins the umbrella commit and NAME only decorates the paths; the
# census records the resolved SHA, which is what the record cites.
REF="${1:?usage: install_bench.sh <ref: tag|branch|sha> <h100|cpu> [name]}"
TARGET="${2:-h100}"
NAME="$(printf '%s' "${3:-$REF}" | tr '/' '-' | cut -c1-16)"
PROJ="${PROJ:-/projects/0/prjs1022/peclet}"
SUITE="$PROJ/suite-$NAME-bench-$TARGET"      # ONE TREE PER BACKEND: a CUDA and a host build share
WHEELS="$PROJ/wheelhouse/$NAME-$TARGET-bench" # neither a checkout, an extern prefix, nor a venv
...
if [ ! -d "$SUITE/.git" ]; then
  git clone https://github.com/computational-chemical-engineering/peclet.git "$SUITE"
fi
cd "$SUITE"
git fetch --tags --force origin
# --detach takes a SHA as happily as a tag; --force makes a re-run of this script idempotent even
# if a previous attempt left the tree dirty.
...
{
  echo "# peclet 1.1.0 scaling benchmark — build census ($(date -Is))"
  echo "host          : $(hostname)   slurm job ${SLURM_JOB_ID:-none}"
  echo "ref requested : $REF   (path name: $NAME)"
  echo "umbrella      : $(git -C "$SUITE" rev-parse HEAD)   $(git -C "$SUITE" describe --tags --always)"
