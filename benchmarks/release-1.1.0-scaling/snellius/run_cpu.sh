#!/bin/bash
# ==========================================================================================
# One rung of the CPU strong-scaling ladder on Snellius genoa (192 cores/node), peclet 1.1.0.
...
find_dir() { local c; for c in "${BENCH:-}" "${SLURM_SUBMIT_DIR:-$PWD}" "${SLURM_SUBMIT_DIR:-$PWD}/.." \
    "/projects/0/prjs1022/peclet/bench-1.1.0"; do
...
TAG_VERSION="${TAG_VERSION:-v1.1.0}"
