...
def _opt(fn, default=None):
    """A diagnostic that may not exist on every build. A run that has already produced its
    timings must not be lost at JSON-write time because a query method was renamed."""
    try:
        return fn()
...
p0(f"[cfg] extent={EXTENT[0]:.4f}x{EXTENT[1]:.4f}x{EXTENT[2]:.4f}  h={H:.6f}  {_geo}  "
   ...
   f"rho={RHO:g} mu={MU:g} F={F:g}  levels_requested={LEVELS}")
...
# ---- solver: the shipped 1.1.0 configuration, on a physical domain --------------------------
