...
counts are flat from 24 to 1536 cores on both cases.
:::
...
With the pressure iteration count flat, the packed-bed step is dominated by the **momentum solve**:
the implicit diffusion's Red–Black Gauss–Seidel hits its sweep cap on every step (600 sweeps per
step, 3 components × 200) because ν·Δt/Δx² ≈ 4×10⁴ on this case, versus 204 sweeps on the
single-phase case. That is the next lever — a velocity multigrid under MPI, or a Krylov wrapper —
and it is why the packed bed's speedup over FoxBerry (3×) is a third of the single-phase one (9–14×)
although the pressure solves are comparable.
