...
`Gatherv`/`Scatterv` per telescoped level per V-cycle. It was opt-in during this campaign; since
2026-09-02 it is the solver **default**, and `Solver.set_pressure_telescope(False)` is the ablation
(the `PECLET_FLOW_TELESCOPE=1` environment variable the runs on this page used was retired in
peclet 1.0.0). `flow.predict_hierarchy` prints the ladder any grid/rank count would get, without
launching anything.
...
why FoxBerry's curve, still on ideal halving there, closes from 25× to 13×. Two caveats belong
with every 1536-core number here: **node placement** — the same configuration measured 0.391 s on
one allocation and 0.256 s on another (a defaults-only verification run), and 0.834 vs 0.786 s on
the bed, so the top rung carries a ±30 % allocation spread that the lower rungs do not — and the
communication-light **Chebyshev pressure driver** brings nothing on this hardware: against a
same-allocation PCG control it is 0.251 vs 0.256 s single-phase (14 iterations either way) and
4.18 vs 0.79 s on the bed (238 iterations against 40; the cut-cell operator's spectrum is not the
interval it assumes). The shipped defaults are slightly more conservative than the diamonds: the momentum tolerance
**follows the pressure solver's** (1e-8 here) rather than a fixed 1e-5, because the projection is
what consumes u* and resolves what the momentum residual leaves to its own tolerance — no free
constant, and the single-GPU regression suite is then identical to its baseline step for step. A
run with no tuning flags at all (telescoping on, coupled tolerance, the velocity-MG auto rule)
measures **3.31 s packed at 384, 0.898 s packed and 0.368 s single-phase at 1536** — 7.3× / 6.1× /
13.8× FoxBerry, `<u>` and `max|div|` to seven digits as above.
