...
That curve decays, and honestly: each rung is a genuinely harder linear system, not just more of the
same, so it mixes parallel efficiency with the problem getting harder. Communication is negligible
there too (0.03–0.77 % of the step), and running one *fixed* grid on 1/2/4/8 GPUs holds iterations at
24.7 / 25.8 / 24.2 / 23.9 — flat — so the decomposition is not the cause.

**These refinement numbers were measured with the smoothed coarse solve**, before the agglomerated
bottom was available. Given what it does to the replicated ladder, re-measuring the refinement ladder
with it is the obvious next step, and the numbers above should be read as an upper bound on the cost
