...
```bash
sbatch --nodes=2 weak_scaling.sh     # the meaningful curve; then strong: sbatch --nodes=2 scaling_test.sh
```

The practical consequence: **the production 182 M run on 4 GPUs (single node, 45 M/GPU) is the sweet
spot** — big enough per GPU to stay compute-bound, and single-node so it pays no inter-node reduction
cost. The hero 614 M run spans 3 nodes and *will* lose efficiency to the multi-node pressure solve (the
suite's known at-scale-tuning frontier). Both drivers now report a **warmup-excluded steady ms/step** and
Mcell-updates/s, so you compare like-for-like.
