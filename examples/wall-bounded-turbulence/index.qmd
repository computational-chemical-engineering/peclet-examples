...
::: {.callout-note}
## GPU-aware MPI on Snellius — possible, but a version knot (default here is host-staging)
The halo can pass **device pointers straight to MPI** (dropping the host-staging copies) — a real win
for multi-node runs. We proved it *works* on Snellius: with `CUDA/12.1.1` + `UCX-CUDA/1.14.1-...-CUDA-12.1.1`
+ `OMPI_MCA_pml=ucx` + `UCX_MEMTYPE_CACHE=n`, the suite's device-pointer send/recv check
([`check_gpu_aware_mpi.sh`](check_gpu_aware_mpi.sh)) passes. **But there's a knot:** the *only* GPU-aware
`UCX-CUDA` on the 2023 module stack is built for **CUDA 12.1.1**, and **Kokkos 5.1.1 requires CUDA ≥ 12.2**
— so you can't build the solver and load that UCX-CUDA at the same time. Hence the scripts default to
**host-staging** (`PECLET_CORE_GPU_AWARE_MPI=0`), which is always correct. To actually enable GPU-aware you
need a stack where *both* hold: either a newer module set (`module -r spider UCX-CUDA` / `module avail
2024` for a `UCX-CUDA` built against CUDA ≥ 12.2 with a matching OpenMPI, then rebuild against it), or a
**user-space CUDA-aware OpenMPI** (UCX 1.20 + OpenMPI 5.0.7 `--with-cuda` against CUDA 12.4, recipe in
`suite/core/docs/cuda-aware-mpi.md`) built in your project space. Two gotchas that cost us time and are
worth remembering: **never pipe `module load`** (it's a shell function; a pipe runs it in a subshell and
the load silently vanishes), and `MPIX_Query_cuda_support()` **over-reports** on this stack — trust the
device send/recv check, not the query.
:::
...
export PECLET_CORE_GPU_AWARE_MPI=0   # host-staged halo (GPU-aware blocked on 2023 stack; see callout)
