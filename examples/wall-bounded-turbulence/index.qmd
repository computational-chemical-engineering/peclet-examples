...
module load 2023 OpenMPI/4.1.5-GCC-12.3.0 Python/3.11.3-GCCcore-12.3.0 CUDA/12.1.1 UCX-CUDA/1.14.1-GCCcore-12.3.0-CUDA-12.1.1     # confirm versions: module avail 2023
...
::: {.callout-tip}
## GPU-aware MPI on Snellius (why CUDA 12.1.1)
The halo can pass **device pointers straight to MPI** (dropping the host-staging copies) — a real win
for the multi-node runs. Snellius' host `OpenMPI/4.1.5` uses a *non*-CUDA UCX by default, which
**segfaults** on device buffers; the fix is to add the **`UCX-CUDA`** module, which only exists built
against **CUDA 12.1.1** — so the whole stack (build + run) pins to 12.1.1 (Hopper/sm_90 is fully
supported there). Validate it with [`check_gpu_aware_mpi.sh`](check_gpu_aware_mpi.sh) (it runs the
suite's device-pointer send/recv and must print *"CUDA-aware MPI works"*); then the run scripts set
`OMPI_MCA_pml=ucx`, `UCX_MEMTYPE_CACHE=n`, and `PECLET_CORE_GPU_AWARE_MPI=1`. Set `=0` to fall back to
host-staging (always correct) if you move to a different MPI. Two gotchas that bite here: **never pipe
`module load`** (it's a shell function; a pipe runs it in a subshell and the load silently vanishes),
and `MPIX_Query_cuda_support()` **over-reports** on this stack — trust the send/recv check, not the query.
:::
...
module load 2023 OpenMPI/4.1.5-GCC-12.3.0 Python/3.11.3-GCCcore-12.3.0 CUDA/12.1.1 UCX-CUDA/1.14.1-GCCcore-12.3.0-CUDA-12.1.1
export GNY=240 GNX=1508 GNZ=503 CFR=15.68 NSTEPS=50000 PECLET_BIND_GPU=1
export OMPI_MCA_pml=ucx UCX_MEMTYPE_CACHE=n PECLET_CORE_GPU_AWARE_MPI=1   # GPU-aware halo (validated); =0 host-stages
