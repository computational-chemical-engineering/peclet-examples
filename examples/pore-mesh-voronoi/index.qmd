...
seeds toward a target volume, and finally `sdf_voronoi_cells` to get the clipped cell polyhedra.

- **1 random** — uniform interstitial seeds, raw Voronoi.
- **2 uniform relaxation** — stage-1 seeds relaxed toward one target volume.
- **3 graded seeding** — seeds with density $\propto 1/V_{\mathrm{ref}}$ (dense at the walls), raw.
- **4 graded relaxation** — stage-3 seeds relaxed toward $V_{\mathrm{ref}}=\phi^3$.

```{python}
#| label: run
N = 4000
opt = dict(method="steepest", max_iter=60, sw=6)                # position-only descent on the walls

s1 = pm.seed_interstitial(centres, radii, L, N, graded=False, seed=1)
...
s3 = pm.seed_interstitial(centres, radii, L, N, graded=True, seed=3)
...
sl = [pm.slice_cells(c, z0) for c in allc]
...
for a, cells, vref, t in [(ax[0], cells2, vset_u, "uniform target"),
    ...
    draw(a, *pm.slice_cells(cells, z0, rel(cells, vref)), rn, "coolwarm", t)
...
- **Denser or coarser.** Raise `N`; the graded seeding density follows $1/V_{\mathrm{ref}}$ automatically.
