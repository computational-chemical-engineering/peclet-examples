...
::: {.callout-warning}
## [1.0.0 OPEN ISSUE] `set_domain_bc_profile` no longer accepts a per-step re-issue
Under peclet.flow 1.0.0, `set_domain_bc_profile`/`set_domain_bc` must be called **before** the
geometry (`set_solid`) is built and now **raise** on any later call — the "re-issue the profile
each step to ramp the jet" pattern described above is no longer supported and there is no
replacement setter for a time-varying inlet profile once geometry exists. The code below is
migrated only as far as the int-\>str codes and the one-time setup order; the per-step re-issue
inside `run_injection`'s loop is left calling the (now-raising) API rather than inventing a
workaround, pending a design decision upstream in `flow`.
:::
...
def run_injection(inject_ms, n_prep=60, n_rise=180, ramp=15, want_snaps=0):
    ...
    dd.initialize_shape('sphere', rp)
    dd.set_domain((0, 0, 0), (NX, NY, NZ)); dd.set_periodic(False, False, False)
    ...
    # [1.0.0: set_domain_bc_profile/set_domain_bc now take str face/type codes, not int, AND raise
    # if called after the geometry (set_solid) is built -- moved ahead of set_solid accordingly.]
    ss.set_domain_bc_profile('-z', inflow_profile(Umf_c)); ss.set_domain_bc('+z', 'outflow')
    ss.set_solid(solid_sdf, True)
    ss.set_pressure_pcg(True, 50, 1e-6)
    ...
    for i in range(total):
        ...
        # [1.0.0 OPEN ISSUE, not fixed here: set_domain_bc_profile now raises once geometry is
        # built (see above), so this per-step re-issue -- which is how the jet is ramped on/off --
        # can no longer run under the 1.0.0 API. There is no replacement setter for a time-varying
        # inlet profile after set_solid as of 1.0.0; this needs a design decision upstream in flow,
        # not a rename. Left as-is (will raise on the first step) rather than inventing a fix or
        # silently freezing the inflow at Umf_c.]
        ss.set_domain_bc_profile('-z', inflow_profile(jv(i)))
        cc.step()
