...
[![Download notebook](https://img.shields.io/badge/Download-notebook-F37626?logo=jupyter&logoColor=white)](https://github.com/computational-chemical-engineering/peclet-examples/blob/main/examples/single-bubble-injection/index.ipynb){target="_blank"}
...
def run_injection(inject_ms, n_prep=60, n_rise=180, ramp=15, want_snaps=0):
    ...
    # Establishing the face TYPES (periodic -> inflow / outflow) must happen before the geometry;
    # once set_solid has run, only a VALUE update (the per-step ramp below) is allowed.
    ...
    for i in range(total):
        ...
        # -z is already 'inflow' (set above, before the geometry), so re-issuing the profile here
        # each step is a VALUE update -- allowed after set_solid -- and is how the jet ramps on/off.
