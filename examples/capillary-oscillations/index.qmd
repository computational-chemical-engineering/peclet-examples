...
[![Download notebook](https://img.shields.io/badge/Download-notebook-F37626?logo=jupyter&logoColor=white)](https://github.com/computational-chemical-engineering/peclet-examples/blob/main/examples/capillary-oscillations/index.ipynb){target="_blank"}
...
```{python}
#| label: wave-driver
def capillary_wave(nx, nu, periods=2.5, rho=1.0, sigma=1.0, cls=flow.Solver):
    ...
    flow.SolverColocated; everything else is identical."""
    nz, ny = nx, 4
    lam = float(nx)
    k = 2 * math.pi / lam
    a0 = lam / 100.0
    ...
    s.set_rho(rho)
    s.set_mu(nu * rho)
    s.set_domain_bc('-z', 'wall', velocity=(0, 0, 0))                     # -z wall
    s.set_domain_bc('+z', 'wall', velocity=(0, 0, 0))                     # +z wall
...
```{python}
#| label: drop-driver
def oscillating_drop(n, R, mu, eps=0.05, periods=2.5, rho=1.0, sigma=1.0, cls=flow.Solver):
    """Mode-2 oscillation of a drop of radius R cells in an n^3 periodic box of the SAME fluid.

    `cls` is the solver class (see the collocated cross-check at the end)."""
...
too, and both drivers above take the solver class as an argument, so the same two
cases run there unchanged. The grid's pressure coupling is an **approximate (ABC)
projection** — average the cell velocities onto a MAC face field, project *that*
exactly, average the correction back — with surface tension applied as a face
acceleration and the cell taking the mean of its two faces. Both cases here are
matched-density, so nothing on this page leans on the part of the rung that is
rated only to density ratio $\lesssim 100$.

```{python}
#| label: colocated-run
CO = flow.SolverColocated
wco = capillary_wave(32, 0.005, cls=CO)
s_ex, _ = wave_mode(wco["k"], wco["nu"], H=wco["nx"] / 2.0)
f = damped_fit(wco["t"], wco["amp"], math.sqrt(wco["k"] ** 3 / 2.0))
wco.update(s_exact=s_ex, w_fit=f["omega"], g_fit=f["gamma"], base=f["offset"])
wco["w_zc"] = omega_from_crossings(wco["t"], wco["amp"], wco["base"])

dco = oscillating_drop(48, 8.0, 0.0025, cls=CO)
f = damped_fit(dco["t"], dco["m2"], dco["w0"])
dco.update(w_fit=f["omega"], g_fit=f["gamma"], base=f["offset"])
dco["s_exact"] = drops[0]["s_exact"]

w0 = waves[0]
print("A. standing capillary wave, 32 cells/lambda, nu = 0.005 "
      f"(exact viscous root omega = {w0['s_exact'].imag:.5f}, gamma = {-w0['s_exact'].real:.3e})")
print(f"{'grid':>12} {'omega fit':>10} {'vs exact':>9} {'gamma fit':>11} {'vs exact':>9}   health")
for lab, r in (("staggered", w0), ("collocated", wco)):
    print(f"{lab:>12} {r['w_fit']:10.5f} {100*(r['w_fit']/r['s_exact'].imag-1):+8.2f}% "
          f"{r['g_fit']:11.3e} {100*(r['g_fit']/(-r['s_exact'].real)-1):+8.1f}%   {r['health']}")

d0 = drops[0]
print(f"\nB. mode-2 drop, 48^3, R = 8, mu = 0.0025 (Lamb omega_0 = {d0['w0']:.5f}, "
      f"exact viscous {d0['s_exact'].imag:.5f})")
print(f"{'grid':>12} {'omega fit':>10} {'vs Lamb':>9} {'vs exact':>9} {'gamma fit':>11}   health")
for lab, r in (("staggered", d0), ("collocated", dco)):
    print(f"{lab:>12} {r['w_fit']:10.5f} {100*(r['w_fit']/r['w0']-1):+8.2f}% "
          f"{100*(r['w_fit']/r['s_exact'].imag-1):+8.2f}% {r['g_fit']:11.3e}   {r['health']}")
DW = 100 * (wco["w_fit"] / w0["w_fit"] - 1)
DD = 100 * (dco["w_fit"] / d0["w_fit"] - 1)
RES = 100 * (dco["w_fit"] / d0["w0"] - 1) - 100 * (dco["s_exact"].imag / d0["w0"] - 1)
```

**The wave.** The two grids land
`{python} f"{abs(DW):.2f}"` % apart, and both sit within
`{python} f"{max(abs(100*(r['w_fit']/r['s_exact'].imag-1)) for r in (w0, wco)):.2f}"` %
of the exact viscous root — so the sharp test of Part A survives the change of
mesh. The decay rates are within
`{python} f"{abs(100*(wco['g_fit']/w0['g_fit']-1)):.0f}"` % of each other and both
carry the same $\approx-15$ % deviation from the exact rate, which is another way
of saying what Part A already said: that deviation belongs to the *scheme's*
under-resolved interfacial boundary layer, not to the grid the momentum lives on.

**The drop, and the open 4 %.** This is the one that matters, because §B's warning
box lists everything the deficit is *not*. The mesh does not join that list, and it
does not resolve it either. The collocated run reads
`{python} f"{100*(dco['w_fit']/dco['w0']-1):.1f}"` % below Lamb against the
staggered `{python} f"{100*(d0['w_fit']/d0['w0']-1):.1f}"` %, and after subtracting
the exact viscous shift the residuals are
`{python} f"{RES:.1f}"` % and
`{python} f"{100*(d0['w_fit']/d0['w0']-1) - 100*(d0['s_exact'].imag/d0['w0']-1):.1f}"` %.
So the deficit **survives a completely different force discretization** — an ABC
face acceleration applied outside the momentum solve, instead of a staggered face
force inside it — which is worth knowing: it is not an artefact of the staggered
pressure coupling. But the grid is not neutral, and saying so is the honest
version: the collocated run is about
`{python} f"{abs(DD):.1f}"` % lower again, and it is lower on the *wave* too
(`{python} f"{100*(wco['w_fit']/wco['s_exact'].imag-1):+.2f}"` % against the exact
root where the staggered grid reads
`{python} f"{100*(w0['w_fit']/w0['s_exact'].imag-1):+.2f}"` %), with a damping
`{python} f"{abs(100*(dco['g_fit']/d0['g_fit']-1)):.0f}"` % below the staggered
one. That is the extra smoothing of the cell-to-face average showing up on both
cases, not a new mechanism — so the change of mesh **adds** to the deficit rather
than explaining it, and the open item stays open.
