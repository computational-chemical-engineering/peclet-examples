...
[![Download notebook](https://img.shields.io/badge/Download-notebook-F37626?logo=jupyter&logoColor=white)](https://github.com/computational-chemical-engineering/peclet-examples/blob/main/examples/hcs-clustering/index.ipynb){target="_blank"}
...
def cool(e, T0=9.0, nsteps=2200, dt=0.02, seed=1):
    ...
    d.initialize_shape('sphere', rp)
    d.set_domain((0, 0, 0), (L, L, L)); d.set_periodic(True, True, True)
...
def dem_box(P, v, e):
    ...
    d.set_domain((0,0,0),(L,L,L)); d.set_periodic(True,True,True); d.set_gravity((0,0,0))
...
dc = dem.Simulation(Nc+64); dc.initialize_shape('sphere', rp)
dc.set_domain((0,0,0),(Lc,Lc,Lc)); dc.set_periodic(True,True,True); dc.set_gravity((0,0,0))
...
- **Fluid.** A volume-averaged incompressible gas: MFIX-Exa advects with the superficial velocity
  ...
  MFIX-Exa by default, "under development"; here via `diagnostics.set_porous_deps_dt(False)`). The momentum is
...
```bash
# Everything on this page runs on the PyPI wheels: the Haff's-law and clustering sections are
# pure peclet.dem, and the gas-damping section's coupling module ships in the cfd-dem extra.
pip install "peclet[cfd-dem]" && quarto render examples/hcs-clustering/index.qmd --execute
# Or from source, against a Kokkos prefix (what a GPU run or a local solver change needs):
