...
through the cooling decay** (within ~30 % down to the clustering plateau at t\* ≈ 1500). The dry
granular system cools visibly slower — the gas is a leading-order energy sink here, not a
perturbation. Both simulations **lift far above the homogeneous theory** as the instability traps
energy in coherent motion. Past the plateau the curves part ways: the benchmark's KE keeps decaying
slowly, while our volume-averaged run's velocity variance *rises* — an identified numerical artifact
of the porous coupling at late times, documented in the callout below; the physical conclusions live
in the decay-and-onset range where the two agree.

```{python}
#| label: fig-mfix-ke
#| fig-cap: "Kinetic-energy decay in the 256-diameter MFIX-Exa system. Left: the benchmark's Figure 33. Right: the lines digitised from it (MFIX sim, red; the benchmark axes span t* = 10–10⁴) plotted together with peclet — the one-for-one gas–solid run at the benchmark's settings (green) and the dry granular run (blue), plus the two analytical kinetic theories. The GTSH theory (black, collisions + drag) reproduces the benchmark's KT line; Haff (grey) is the collisions-only limit. The gas–solid run tracks the digitised MFIX sim through the cooling decay to the clustering plateau; the dry system cools visibly slower. Past the plateau our volume-averaged run's variance rises (late-time coupling artifact — see the callout) while the benchmark's decays slowly."
...
still faint at t\* = 1000 (cluster index ≈ 1.3), sharpens into a many-filament network through
t\* = 2000–3000 (index 2.2 → 3.0), and peaks near t\* ≈ 5600 (index 3.3) — the same
branching-filament morphology as the benchmark panel.

```{python}
#| label: fig-mfix-xy
#| fig-cap: "Particle positions projected onto the x–y plane. Top: the MFIX-Exa Figure 34 — Goldhirsch–Zanetti 1993 (a 2-D hard-disk system) on the left, MFIX-Exa's own gas–solid result (labelled t* = 1000) on the right. Middle: the peclet one-for-one gas–solid run through its development window — the filament network builds from t* = 2000 to its peak near 5000, the benchmark panel's morphology. Bottom: the dry granular run for contrast — the same instability, developing later and coarsening more slowly."
...
axtop.set_title("MFIX-Exa — Figure 34   (Goldhirsch–Zanetti 2-D reference | MFIX-Exa gas–solid result)",
                fontsize=10)
for k, tst in enumerate((2000, 3000, 5000)):
...
- **Fluid.** A volume-averaged incompressible gas: MFIX-Exa advects with the superficial velocity
  ...
  MFIX-Exa by default, "under development"; here via `set_porous_deps_dt(False)`). CFD grid
  Δ\* = 2 particle diameters, per their decks and grid heuristic.
- **What remains — a stage offset.** The fully-filamented state in the benchmark's Fig. 34 panel,
  labelled t\* = 1000, occurs in our run at t\* ≈ 3000–5000, with the same branching morphology.
  ...
  material; what the matched cooling history, onset ordering, and morphology establish is that the
  instability physics is reproduced.
- **What remains — a late-time energy artifact (open item).** Past the clustering plateau
  (t\* ≳ 2500) the particle velocity variance in our volume-averaged run *rises* instead of
  continuing the benchmark's slow decay, eventually re-melting the clusters. The rise has no physical
  source — it exceeds the *total* gas kinetic energy many times over — so it is numerical energy
  injection in the porous + implicit-drag exchange, and it is insensitive to the ∂ε/∂t constraint
  term (both variants measured). The incompressible-gas variant of the same run shows no rise. The
  energy bookkeeping of the split "advance momentum → project onto the moving ε-constraint" is the
  suspect and is under investigation; all comparisons above are drawn from t\* ≲ 3000, where the
  artifact is small.
:::
