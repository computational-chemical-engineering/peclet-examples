...
```{python}
#| label: load-mfix
#| code-summary: "Load the cached MFIX-scale run + the parameter-free Enskog kinetic-theory curve"
...
# Parameter-free kinetic theory (homogeneous cooling): √(T₀/T) = 1 + (Enskog slope)·t  ⇒  Haff's law,
# the same collisional rate validated against Enskog above. The gas drag adds < a factor over t*=1000
# at ρ_p/ρ_g = 1000, so this collisional law is the KT baseline the clustering peels away from.
kt = T0m/(1 + esm*ts)**2
print(f"MFIX-scale run: N={int(data['N'])}, φ={float(data['phi']):.3f}, e={float(data['e'])}, "
      f"to t*={ts[-1]:.0f}: T/T₀={Trat[-1]:.2e} (KT {kt[-1]:.2e}, {Trat[-1]/kt[-1]:.0f}× above); "
      f"clustering index {cidx[0]:.2f} → {cidx[-1]:.2f}")
```

### Kinetic-energy decay — our Figure 33

`peclet` follows the parameter-free kinetic theory during homogeneous cooling, then **lifts above it**
as the clustering instability grows — the coherent motion trapped inside the clusters stops the granular
temperature cooling as fast as homogeneous theory predicts. That is exactly the MFIX-Exa Figure 33
signature (their `sim` peeling off the `KT` line). The gas-coupled points (Re~T0~ = 20) sit just below
the dry curve — the modest extra damping the benchmark's drag adds.

```{python}
#| label: fig-mfix-ke
#| fig-cap: "Kinetic-energy decay in the 256-diameter MFIX-Exa system, over the benchmark's full t* range. Left: MFIX-Exa Figure 33 (their `sim`, red, against the `KT` kinetic theory, black). Right: peclet — T/T₀ = KE/KE₀ against the parameter-free Enskog/Haff kinetic theory (black). The simulation tracks the theory during homogeneous cooling and then rises above it as the clusters form and hold their energy; the gas-coupled run (Re_T0 = 20, orange) adds only a modest extra damping."
...
m = ts >= 10
ax[1].loglog(ts[m], kt[m], color="k", lw=1.8, label="KT (Enskog/Haff theory)")
ax[1].loglog(ts[m], Trat[m], color="#d1210b", lw=1.8, label="peclet — dry (sim)")
gm = gasd["ts"] >= 10
ax[1].loglog(gasd["ts"][gm], gasd["Trat"][gm], color="#e8862a", lw=1.5, ls="--",
             label="peclet — gas, Re$_{T0}$=20")
ax[1].set(xlabel=r"$t^* = t\,\sqrt{T_0}/d_p$", ylabel=r"$T/T_0$  or  $KE/KE_0$",
          xlim=(10, 1e4), ylim=(1e-5, 1), title="peclet — this run")
ax[1].legend(loc="lower left", fontsize=8); ax[1].grid(True, which="both", alpha=0.15)
plt.tight_layout()
```

### The clusters — our Figure 34

Projected onto the *x–y* plane, the 50 000 grains organise into the branching **filaments and voids** of
the clustering instability. It coarsens with time exactly as the benchmark shows: at t\* = 5000 the
texture matches their simulation panel, and by t\* = 10 000 the sharp filaments match the
Goldhirsch–Zanetti reference.

```{python}
#| label: fig-mfix-xy
#| fig-cap: "Particle positions projected onto the x–y plane. Top: the MFIX-Exa Figure 34 (their simulation on the left, the Goldhirsch–Zanetti 1993 reference on the right). Bottom: peclet — the 50 000 grains at t* = 5000 (matching their simulation) and t* = 10 000 (matching the Goldhirsch–Zanetti filaments)."
fig = plt.figure(figsize=(11, 8.4))
gs = fig.add_gridspec(2, 2, height_ratios=[1.0, 1.25], hspace=0.12, wspace=0.06)
...
for k, tst in enumerate((5000, 10000)):
    a = fig.add_subplot(gs[1, k]); xy = data[f"xy_{tst}"]
    a.scatter(xy[:, 0], xy[:, 1], s=0.5, c="#111", edgecolors="none")
