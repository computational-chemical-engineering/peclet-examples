...
Re_T0 = 20). Saves hcs_gas_bvk_h2.npz with the T(t*) trace, clustering index and x-y snapshots.
...
s = flow.Solver(Lx//2, Ly//2, Lz//2); s.set_rho(rho_g); s.set_mu(mu_g); s.set_dt(dt)
for f in range(6): s.set_domain_bc(f, 0)     # all periodic
s.set_pressure_pcg(True, 30, 1e-6)
cpl = CfdDem(s, d, fluid_dt=dt, mu=mu_g, rho=rho_g, radius=rp, drag="beetstra", h=2.0,
...
for i in range(nsteps):
    ...
    if (i+1) % 10000 == 0:
        ...
        np.savez("hcs_gas_bvk_h2.npz", ts=np.array(ts), Trat=np.array(Trat), cidx=np.array(ci),
...
np.savez("hcs_gas_bvk_h2.npz", ts=np.array(ts), Trat=np.array(Trat), cidx=np.array(ci),
         ...
         **{f"xy_{k}": (vv if vv is not None else np.zeros((0, 2))) for k, vv in snaps.items()})
print("saved hcs_gas_bvk_h2.npz", flush=True)
