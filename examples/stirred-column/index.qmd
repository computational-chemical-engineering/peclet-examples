...
`set_wall_transform` and `diagnostics.wall_sdf_at` are new. The CSG authoring layer is
`peclet.geom`, which `pip install peclet` provides as a wheel from peclet 1.2.0 — before that it was
`peclet.core.geom` in the sdist-only `peclet-core`, which needed an MPI toolchain
(suite/docs/CORE_BOUNDARY.md). The page is *frozen*, so it renders and reads correctly either way.
:::
