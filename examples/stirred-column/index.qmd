...
`set_wall_transform` and `diagnostics.wall_sdf_at` are new, and the CSG authoring layer
...
`diagnostics.wall_sdf_at` returns exactly what the narrow phase reads, so it is the honest way to check a
...
def stirrer_mesh(sim, wall, box=(-R_IMP-1, R_IMP+1), ylim=(0.0, 8.0), n=110):
    ...
    fld = np.asarray(sim.diagnostics.wall_sdf_at(wall, pts)).reshape(X.shape)
...
with `diagnostics.wall_sdf_at`, not with a hand-written formula that could disagree with the collision geometry.
...
def fill(sim, w_stir, fill_top=11.0, seed=4):
    ...
    P = np.ascontiguousarray(P[np.asarray(sim.diagnostics.wall_sdf_at(w_stir, P)) > RP*1.15])
    sim.set_positions(P)
...
sdf_f = np.asarray(sim.diagnostics.wall_sdf_at(w_col, np.ascontiguousarray(Qf.astype(np.float32))))
...
CHECK = textwrap.dedent("""
    ...
    sim.set_gravity((0.0, -G, 0.0)); sim.initialize_shape('sphere', 1.0); sim.set_global_scale(RP)
    ...
    P = P[np.hypot(P[:, 0], P[:, 2]) < R_COL - 1.3*RP]
    P = np.ascontiguousarray((P + rng.normal(0, 0.06*RP, P.shape)).astype(np.float32))
    P = np.ascontiguousarray(P[np.asarray(sim.diagnostics.wall_sdf_at(ws, P)) > RP*1.15])
