...
[![Download notebook](https://img.shields.io/badge/Download-notebook-F37626?logo=jupyter&logoColor=white)](https://github.com/computational-chemical-engineering/peclet-examples/blob/main/examples/random-packed-bed/index.ipynb){target="_blank"}
...
def pack_bed(N=180, phi_ref=0.63, radius=0.5, seed=3, dt=0.002, limit_time=7.0, iters=60,
    ...
    s.set_growth_params(gr, scale_init); s.set_thermostat(temperature, dt); s.set_dt(dt)

    def overlap_frac():
        c = 2 * radius * float(s.get_scales().ravel().mean())
        return float(s.compute_overlaps()) / max(c, 1e-9)

    # MONOTONIC growth. When overlaps build, we RELAX them at the current size (relax() steps grow
    ...
    for step in range(int(limit_time / dt)):
        ...
        s.step(); mo = overlap_frac()
        gf = float(s.growth_factor)
        if mo > criterion:                                  # overlaps too big -> relax at CURRENT size
            it = 0; prev = mo
            while it < 40:
                s.relax(); it += 1; mn = overlap_frac()   # relax(): pure overlap removal, no growth
    ...
    r_eff = radius * s.get_scales().ravel() * float(s.growth_factor)
