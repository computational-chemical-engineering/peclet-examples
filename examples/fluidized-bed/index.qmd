...
::: {.callout-important}
## Sub-cell grains need `set_global_scale`
These grains are 0.2 cells across, but the DEM's default length scale (`global_scale = 1`) also sets
its **ghost band**. Leaving it at 1.0 makes the ghost band **ten times** the grain — the broadphase
chokes. Build the sphere at canonical radius 1 and scale the whole simulation to `rp`, so the effective
radius *and* the ghost band track the grain size. (Single-phase DEM examples avoid this by working in
grain-radius units — radius 1 — where the default is already right.)
:::
...
def make_dem(pos):
    ...
    d.initialize(shape_type=1, radius=1.0)       # sphere (canonical); scaled to rp next
    d.set_global_scale(rp)                        # effective radius = rp; ghost band ~ rp
...
The *coupled* step is heavier — the CFD solve, the volume deposition and the drag gather add a fixed
per-step cost — so a million-grain fluidized bed runs at ~1 s/step on the GPU rather than 10 ms. In
other words the coupling overhead, not the DEM, sets the coupled cost; the DEM has headroom to spare.
