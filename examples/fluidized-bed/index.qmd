...
subtitle: "Blow gas up through a cylinder of grains until the bed unpacks and boils — the two solvers exchanging drag and void fraction every step — then run the real thing: a million 1 mm glass beads in air at 2.0 U~mf~, on the GPU, with a movie."
author: "Peclet"
date: "2026-07-06"
...
3. Run it at engineering conditions: **one million 1 mm glass beads in air at 2.0 U~mf~**, check the
...
(ρ~g~ = 1.2 kg/m³, μ = 1.8·10⁻⁵ Pa·s), driven at **2.0 × U~mf~**. Minimum fluidization from the
...
U_si  = 2.0 * U_mf
print(f"Ar = {Ar:.3g}   Re_mf = {Re_mf:.1f}   U_mf = {U_mf:.2f} m/s   U = 2.0 U_mf = {U_si:.2f} m/s")
```
...
# A static column carries its weight through contact chains, and the position solve propagates a
# correction roughly one particle layer per sweep — this bed is ~110 layers of d_p deep, so give the
# solve an iteration budget of that order for the settling transient (the adaptive stop exits early
# once converged, so the high cap costs little afterwards).
dB.set_solver_iterations(120, 4)

t0 = time.time()
for _ in range(4000):                            # 0.2 s of pure DEM settling
    dB.step(dt / 20)
pz = dB.get_positions()[:Nbig, 2]
print(f"settled in {time.time()-t0:.0f}s: bed top z95 = {np.percentile(pz,95)*h_m*100:.1f} cm")
```

Fluidize at 2.0 U~mf~ for **1.5 seconds** of physical time, recording a movie frame every 10 ms and
the two key diagnostics — the **bed pressure drop** and the **bed height** — as it runs. The textbook
check for a fluidized bed is `ΔP ≈ M·g/A` — once fluidized, the gas carries the weight of the solids.
...
frames, dPs, beds = [], [], []
steps, every = 1500, 10                                           # 1.5 s, frame every 10 ms
t0 = time.time()
...
```{python}
#| label: fig-si-diag
#| fig-cap: "The engineering checks. Left: after the startup slug's ΔP spike, the bed pressure drop oscillates with the bubbling around the bed-weight line — the gas carries the solids (the printed ratio above is the quantitative check). Right: the bed surface bubbles around its working height."
...
A **D/d~p~ = 100** column at 2.0 U~mf~ is a vigorously bubbling/slugging bed (1 mm glass sits near the
