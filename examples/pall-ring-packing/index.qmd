...
The CSG authoring layer is `peclet.geom`, which `pip install peclet` provides as a wheel from
peclet 1.2.0 — before that it was `peclet.core.geom` in the sdist-only `peclet-core`, which needed
an MPI toolchain (suite/docs/CORE_BOUNDARY.md). Composed analytic particles
(`Simulation.add_scene_shape`) and the `peclet.dem.scene_particle` pipeline need peclet-dem from the
same family. The page is *frozen*, so it renders and reads correctly either way.
:::
