...
already carries — we transform each bean's shell by its final position and orientation
and scatter the points, coloured by height:
...
ax.set_title(f"N = {N} beans,  phi ~ {phi:.2f}")
...
- **Export the packing.** `sim.write_vtp("beans.vtp")` writes particle centres/orientations
