...
def pack_ring_bed(N=46, D0=1.0, aspect=1.5, wall_ratio=0.4, target_phi=0.50, seed=0,
    ...
    s = dem.Simulation(N)
    s.initialize_shape(shape_type=2, radius=R, height=H, thickness=t)   # shape_type 2 = hollow cylinder
    ...
    for step in range(steps):
        ...
        s.step(dt); ov = float(s.max_overlap())
        gf = float(s.get_growth_factor())
        if ov > jam:                                       # overlaps too big -> relax at CURRENT size
            it = 0; prev = ov
            while it < 30:
                s.step(0.0); it += 1; ro = float(s.max_overlap())   # dt = 0: pure overlap removal
